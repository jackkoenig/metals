...
class SymbolIndexBucket(
    toplevels: TrieMap[String, (s.SymbolInformation.Kind, Set[AbsolutePath])],
  ...
  def toplevelsAt(path: AbsolutePath): List[SymbolDefinition] = {
    val pathSymbolInfos = if (path.isSourcesJar) {
      path.listRecursive.toList.map { source =>
  ...
  def addIndexedSourceJar(
    ...
    if (sourceJars.addEntry(jar.toNIO)) {
      ...
      patched.foreach { case (sym, path) =>
        val (kind, acc: Set[AbsolutePath]) = toplevels.getOrElse(
          sym,
          (s.SymbolInformation.Kind.UNKNOWN_KIND, Set.empty)
        )
        toplevels(sym) = (kind, acc + path)
      }
    }
  }
  ...
  def addSourceFile(
    ...
    patched.foreach { symbol =>
      ...
      toplevels(symbol) = (kind, acc + source)
    }
    symbols
  }
  ...
  def addToplevelSymbol(
    ...
    if (source.isAmmoniteScript || !isTrivialToplevelSymbol(path, toplevel)) {
      val (_, acc: Set[AbsolutePath]) =
        toplevels.getOrElse(toplevel, (info.kind, Set.empty))
      toplevels(toplevel) = (info.kind, acc + source)
    }
  }
        ...
        if (toplevel.value.endsWith(".")) {
          ...
          for {
            companionClassFile <- toplevels
              ...
              .toSet
              .flatten
            if (!files.exists(_._2.contains(companionClassFile)))
  ...
  private def removeOldEntries(symbol: Symbol): Unit = {
    val exists =
      (toplevels.get(symbol.value).map(_._2).getOrElse(Set.empty) ++ definitions
        .get(symbol.value)
        .map(_.map(_.path))
    ...
    toplevels.get(symbol.value) match {
      ...
      case Some((kind, acc)) =>
        ...
        else toplevels(symbol.value) = (kind, updated)
    }
